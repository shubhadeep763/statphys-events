...
### Automatic (weekly)
A GitHub Action (`.github/workflows/update-events.yml`) runs every Monday at
03:00 UTC. It calls the Claude API (`scripts/update-events.mjs`) with web
...
run manually from the Actions tab ("Weekly event update" → Run workflow), or
